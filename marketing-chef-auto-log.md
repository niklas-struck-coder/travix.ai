...
**Repo-Zustand zu Beginn des Laufs:** `marketing-chef/auto`
(`origin/marketing-chef/auto`) war auf `7fe3f0c` (21.09., weiterer Lauf)
hängengeblieben, dessen Inhalt laut `freigabe-chef-log.md` ("2026-09-21
weiterer Lauf") bereits vollständig in `main` gemergt war — der Branch
war also nur noch veraltet, nicht mehr in Arbeit. Neu von aktuellem
`origin/main` (`6dae998`) aus angelegt.
...
**Warum dieser Punkt:** Erst geprüft, ob eine der vier offenen Fragen aus
`marketing/freigabe-uebersicht.md` seit dem 21.09. beantwortet wurde:
keine Notiz in `status.md` (Stand weiterhin 21.09.), `ZEITPLAN.md` (6.2
weiterhin `[ ]`) oder diesem Dokument, keine neuen Kanal-Links im Repo,
kein `changelog`-Treffer in `src/routes.tsx`. Alle vier Fragen bleiben
offen — jetzt seit über fünf Wochen. Danach `git log dc451a8..origin/main`
geprüft: neun neue Commits, keiner davon mit echter Produkt-Codeänderung
in `main` — nur Berichte, Logs und ein Status-Update (per `git show
--stat` je einzeln bestätigt). Ein IT-Chef-Bericht erwähnt zwar PR #22
(role="alert"-Fix), der ist laut `git log main..origin/it-chef/auto`
aber weiterhin nicht gemergt und zählt deshalb nach der etablierten Regel
noch nicht. Der Tier-4-Kandidatentopf bleibt bei eins (seit dem
21.09.-Update) — klar unter der Achter-Schwelle für eine neue
Mini-Changelog-Ausgabe. Details siehe `marketing/freigabe-uebersicht.md`,
Update 2026-09-22.
...
--is-ancestor` bereits vollständig in `main` gemergt war (Merge-Commit
`34e43e9`). Neu von aktuellem `origin/main` (`2309d4b`) aus angelegt.
...
**Warum dieser Punkt:** Erst geprüft, ob eine der vier offenen Fragen aus
`marketing/freigabe-uebersicht.md` ("Nächster autonomer Lauf") seit dem
11.09. beantwortet wurde: keine Notiz in `status.md` (Stand weiterhin
09.09.), `ZEITPLAN.md` (6.2 weiterhin `[ ]`) oder diesem Dokument, keine
neuen Kanal-Links im Repo, kein IT-Chef-Commit zur Mini-Changelog-Seite.
Alle vier Fragen bleiben offen. Danach `git log 34e43e9..origin/main`
geprüft: 11 neue Commits, davon keiner content-relevant — ein
Freigabe-Chef-Log (bestätigt, dass dieser Branch seit dem letzten Merge
nicht angefasst wurde), fünf reine Testdatei-Nachzüge ohne
Verhaltensänderung (per Commit-Beschreibung und Diff verifiziert), ein
Support-Chef-Bericht, der eigene interaktive Marketing-Chef-Bericht vom
11.09., ein IT-Chef-Bericht ("kein neuer Bug gefunden", per `git show`
verifiziert nur `reports/it-chef.md` geändert), ein Daily-Status-Update
und ein weiteres Freigabe-Chef-Log. Keine einzige neue Produkt-
Codeänderung, also kein fünfter Tier-4-Kandidat — der Topf bleibt bei
vier (Stand 11.09.).
...
**Andere Punkte geprüft und bewusst nicht gewählt:**
- Ein neuntes eigenständiges Social-Content-Stück bzw. eine dritte
  Mini-Changelog-Ausgabe — Kandidatentopf bleibt bei vier, weiterhin auf
  der Menge, die selbst am 06.09. als "nicht ausreichend" galt.
- "Landingpage/Warteliste live" (Sprint 2), "Community/Warteliste
  aufbauen" (Sprint 4), Testkampagnen/Launch-Kampagne (Sprint 6/7) —
  weiterhin Live-Vorgänge bzw. an ungelöste Freigabe-Fragen gebunden.

**Umgesetzt:**
- `marketing/freigabe-uebersicht.md`: neues Update vom 12.09. (Prüfung
  der vier Fragen, Verifikation der 11 neuen Commits als nicht
  content-relevant), "Nächster autonomer Lauf"-Abschnitt aktualisiert.
...
neuen Tier-4-Kandidaten ergänzt, kein neues Content-Stück und keine dritte
Mini-Changelog-Ausgabe.

**Warum dieser Punkt:** Erst geprüft, ob eine der vier offenen Fragen an
Ni seit dem 10.09. beantwortet wurde (Vorgabe aus
`marketing/freigabe-uebersicht.md`, Abschnitt "Nächster autonomer Lauf"):
keine Notiz in `status.md` (Stand weiterhin 09.09.), `ZEITPLAN.md` oder
diesem Dokument, 6.2 weiterhin `[ ]`, keine neuen Kanal-Links, kein
IT-Chef-Commit zur Mini-Changelog-Seite. Alle vier Fragen bleiben offen.
Danach `git log 5d056c2..origin/main` geprüft: 10 neue Commits, neun davon
ohne Content-Relevanz (ein Freigabe-Chef-Log, ein Daily-Status-Update, der
...
vier reine Testdatei-Nachzüge ohne Verhaltensänderung). Eine echte, per
`git show` verifizierte Codeänderung geprüft: `78c764f` (11.09., später
Commit) setzt den kurzfristigen Teil von Vorschlag 3 aus
`reports/support-chef.md` (10.09.) um — "Planung fortsetzen" auf der
Reiseentwürfe-Seite verlinkte bisher bei jedem Entwurf identisch auf
denselben einen aktiven Chat, ohne erkennbar zu machen, dass das nicht die
Details des jeweiligen Entwurfs öffnet; jetzt zeigt die Seite bei mehr als
einem Entwurf einen ehrlichen Hinweis darauf. Echter, angebundener
Nutzerpfad, daher als neuer Tier-4-Kandidat aufgenommen. Mit nur einem
neuen Kandidaten (Topf wächst damit von drei auf vier seit Ausgabe 2) ist
die eigene Gating-Bedingung für eine dritte Ausgabe weiterhin nicht klar
erfüllt — die Menge liegt exakt auf dem Stand, der selbst am 06.09.
ausdrücklich als "nicht ausreichend" bewertet wurde.
...
- Ein neuntes eigenständiges Social-Content-Stück bzw. eine dritte
  ...
  Tier-4-Kandidat seit dem 10.09., exakt auf der Menge, die am 06.09.
  bereits als "nicht ausreichend" galt.
- Die drei am 09.09. gemeldeten, noch offenen Support-Chef-Punkte
  (Löschen ohne Bestätigung, Warenkorb-Sackgasse, Entwurf-Fortsetzen
  generisch) als eigene Tier-4-Kandidaten — laut Support-Chef-Bericht vom
  10.09. weiterhin offen; der heutige Fund deckt nur den kurzfristigen,
  nicht-strukturellen Teil eines der drei Punkte ab, kein vollständig
...
**Umgesetzt:**
- `marketing/freigabe-uebersicht.md`: neues Update vom 11.09. (Prüfung der
  vier Fragen, Einordnung des neuen Commits, Begründung gegen eine dritte
  Ausgabe), Tier-4-Abschnitt und "Nächster autonomer Lauf"-Abschnitt
  aktualisiert.
