...
- [x] 3.0 Core layout & navigation — Sidebar, route shells, responsive shell
  ...
  - [x] 3.3 Create `MobileNav.tsx` with hamburger menu / bottom nav for mobile — vom autonomen IT-Chef-Lauf am 18.09. (weiterer Lauf) eine Fokus-Parität in `src/components/ui/sheet.tsx` nachgezogen (das `MobileNav.tsx` als Menü-Container nutzt): `SheetContent` bekam denselben `onOpenAutoFocus`/`onCloseAutoFocus`-Fallback wie `DialogContent` (Fokus zur Seiten-`<h1>`, falls der öffnende Auslöser beim Schließen schon aus dem DOM entfernt wurde), bisher unbemerkt, weil `MobileNav.tsx`s Trigger nie entfernt wird. Neue `sheet.test.tsx`. Vom autonomen IT-Chef-Lauf am 18.09. (weiterer Lauf) einen von `reports/support-chef.md` (18.09., Vorschlag 1) gemeldeten Reibungspunkt behoben: Genau dieser neue Fokus-Fallback schickte den Fokus nach jedem Klick auf einen Menüpunkt zurück zum Hamburger-Knopf (bleibt als `SheetTrigger` nach jeder Navigation im DOM) statt zur `<h1>` der neu geladenen Seite — Tastatur-/Screenreader-Nutzer:innen mussten sich nach jedem Menüpunkt erneut durch Kopfzeile und Menü zur Seite vorarbeiten. Fix: `sheet.tsx` exportiert die Fokus-auf-`<h1>`-Hilfsfunktion jetzt als `focusPageHeading()`; `MobileNav.tsx` merkt sich per `navigatedRef`, ob das Schließen durch einen echten Navigationsklick ausgelöst wurde, und ruft in diesem Fall in einem eigenen `onCloseAutoFocus` `focusPageHeading()` statt des generischen Trigger-Fallbacks auf. Schließen ohne Navigation (Escape, Overlay-Klick, X-Knopf) kehrt weiterhin unverändert zum Hamburger-Knopf zurück. Zwei neue Regressionstests in `MobileNav.test.tsx`.
...
- [ ] 5.0 Real search modules — Hotel cards, train/bus/ferry search, Duffel flight wizard stub
  ...
  - [x] 5.2 Build `HotelResults.tsx` grid showing minimum 3 comparable hotel cards — grid component built and used for inline results inside `KiChat.tsx`; the standalone `/hotelsuche` page renders its own equivalent grid directly (see 5.9-analog note in `Flugsuche.tsx`). Vom autonomen IT-Chef-Lauf am 21.09. (weiterer Lauf) ergänzt: der Ladehinweis ("Travix sucht echte Unterkünfte …", direkt neben dem im selben Lauf reparierten `isThinking`-Block in `KiChat.tsx`) bekam `role="status"`, analog `FlightResults.tsx`/`TrainResults.tsx`, siehe `ZEITPLAN.md` (21.09.-Eintrag). Vom autonomen IT-Chef-Lauf am 21.09. (vierter Lauf desselben Tages) ergänzt: der Fehlerzustand (fehlgeschlagene Suche) bekam `role="alert"`, analog `FlightResults.tsx`, siehe `ZEITPLAN.md` (21.09.-Eintrag). Vom autonomen IT-Chef-Lauf am 21.09. (fünfter Lauf desselben Tages) dieselbe Lücke in der standalone `Hotelsuche.tsx`-Seite selbst nachgezogen (nicht in `HotelResults.tsx`) — siehe 5.9-Eintrag.
  ...
  - [x] 5.4 Build `TrainCard.tsx` with departure/arrival, times, duration, transfers, operator, price, classes — vom autonomen IT-Chef-Lauf am 18.09. um eine `selected`-Prop ergänzt (fehlte bisher anders als bei `FlightCard`/`HotelCard`: Button blieb nach Auswahl unverändert klickbar, kein "Ausgewählt"-Zustand), 1:1 nach demselben Muster wie in den beiden anderen Karten
  - [x] 5.5 Build `TrainResults.tsx` list view for train/bus/ferry connections — vom autonomen IT-Chef-Lauf am 21.09. (weiterer Lauf) ergänzt: der Ladehinweis bekam `role="status"`, analog `FlightResults.tsx`/`HotelResults.tsx`, siehe `ZEITPLAN.md` (21.09.-Eintrag)
  ...
  - [ ] 5.7 Integrate hotel and train search results into KI-Chat response rendering — hotel half done (`HotelResults` wired into `KiChat.tsx`); train/bus/ferry half still blocked, no connection-search data source is integrated (Duffel has none) and picking a provider is a product decision for Ni. Nebenbei am 28.08. vom autonomen IT-Chef-Lauf (vierter Lauf) behoben: `mockAdvisor.ts`'s final chat step used to promise a real search ("Ich suche jetzt nach echten X-Verbindungen") for every transport mode, but only flight ever actually searches — train/bus/ferry/car left the user hanging. Now only flight keeps that promise; the other modes get an honest completion message instead. Does not close this checkbox — the actual train/bus/ferry search integration is still missing.
  - [x] 5.8 Build `FlightWizard.tsx` with IATA code inputs, passenger details, cabin class, one-way/round-trip toggle
  - [x] 5.9 Create Flugsuche page (`/flugsuche`) hosting the flight wizard — vom autonomen IT-Chef-Lauf am 21.09. (dritter Lauf) nachgebessert: `<NoResultsMessage />` ohne `title`-Prop ließ bei leeren Suchergebnissen den generischen Text "Keine Ergebnisse gefunden" statt "Keine Flüge gefunden" anzeigen — anders als bei `Hotelsuche.tsx`, `FlightResults.tsx`, `HotelResults.tsx` und `TrainResults.tsx`, die den Titel alle explizit setzen. `title="Keine Flüge gefunden"` ergänzt, analog zum bereits bestehenden Muster in den Geschwister-Komponenten. Neuer Regressionstest in `Flugsuche.test.tsx`. Vom autonomen IT-Chef-Lauf am 21.09. (fünfter Lauf desselben Tages) den im vierten Lauf bewusst zurückgestellten Fund nachgeholt: Der Fehlerzustand (fehlgeschlagene Suche) in `Flugsuche.tsx` und `Hotelsuche.tsx` selbst (nicht `FlightResults.tsx`/`HotelResults.tsx`, die bereits im vierten Lauf repariert wurden) hatte ebenfalls kein ARIA-Live-Region-Attribut. `role="alert"` auf beiden Fehler-`<div>`s ergänzt, je ein neuer Regressionstest in `Flugsuche.test.tsx`/`Hotelsuche.test.tsx`, siehe `ZEITPLAN.md` (21.09.-Eintrag, fünfter Lauf).
...
- [ ] 6.0 Interactive trip plan — BuchungsSeite, checklist, cost breakdown, provider links
  ...
  - [x] 6.8 Build `ChecklistPanel.tsx` with 13 checklist items (outbound/return flight, accommodation, activities, etc.) — `src/components/trip/ChecklistPanel.tsx`, wired into `Buchung.tsx` below the section grid; single "Transport" item instead of separate outbound/return since `TripDraft` only tracks one `transportMode` field, see `ZEITPLAN.md` 6.8/6.9 for the full item list
  - [x] 6.9 Implement `checklistRules.ts` for auto-detecting completion from trip data — `src/lib/trip/checklistRules.ts`, 5 items auto-detected from the same fields as `calculateProgress.ts`, 8 standard prep items checked off manually (no matching `TripDraft` field for those)
  - [x] 6.10 Wire checklist items to open KI-Chat for planning that specific component — vom autonomen IT-Chef-Lauf am 26.08. (fünfter Lauf) umgesetzt: die fünf automatisch erkannten Zeilen in `ChecklistPanel.tsx` sind jetzt Links zu `/ki-chat?edit=<feld>` — dasselbe Muster, das die Section-Karten in `Buchung.tsx` schon für Transport/Reisedaten/Budget/Unterkunft nutzen. `activities` hat kein eigenes `?edit=`-Feld in `useChat.ts` (`editableFields` kennt nur transportMode/dates/budget/accommodation), verlinkt daher auf einen neuen Chat statt eines Edit-Parameters. Die acht manuellen Vorbereitungspunkte (Reisepass, Visum, …) bleiben unverändert reine Ankreuz-Buttons, da sie keinem planbaren Trip-Feld entsprechen. Vom autonomen IT-Chef-Lauf am 27.08. (vierter Lauf) nachgebessert: Support-Chef meldete, dass die Links optisch nicht von den abhakbaren Zeilen unterscheidbar waren — jetzt mit `Pencil`-Icon (analog `Buchung.tsx`) und `sr-only`-Zusatz ", bearbeiten" im Linktext.
  ...
  - [x] 6.12 Build `EditMode.tsx` for manual add/remove activities and price adjustments — dialog in `src/components/trip/EditMode.tsx`, wired into the "Aktivitäten" section on `Buchung.tsx`; add/remove/price-edit persist via the existing `updateStoredTrip()` (`tripStorage.ts`), no new data field needed since `TripActivity` already has `name`/`price`. Scoped to activities only, without a category cost summary — that's 6.6/6.7 (`CostBreakdown.tsx`/`calculateCosts.ts`), still blocked because `TripDraft` has no price field for transport/accommodation yet. Vom autonomen IT-Chef-Lauf am 29.08. (achter Lauf) nachgebessert: Name-/Preisfeld im "Neue Aktivität"-Formular hatten keinen Enter-Handler, anders als `ChatInput.tsx`s etabliertes Muster (Enter löst Senden aus) — jetzt lösen beide Felder beim Drücken von Enter `addActivity()` aus, gleiche Leer-Namen-Schutzbedingung wie der bestehende Button. Vom autonomen IT-Chef-Lauf am 16.09. (fünfter Lauf desselben Tages) nachgebessert: Der Entfernen-Button löschte eine Aktivität bisher sofort ohne Rückfrage, anders als dasselbe Löschen auf `/aktivitaeten` — jetzt fragt derselbe Bestätigungsdialog wie dort ("Aktivität entfernen?"/"Ja, entfernen"/"Abbrechen") vor dem endgültigen Entfernen nach. Vom autonomen IT-Chef-Lauf am 20.09. (weiterer Lauf) nachgebessert: Preis-Input und Entfernen-Button jeder Aktivität hatten ihr aria-label nur aus `activity.name` gebildet — zwei gleichnamige Aktivitäten (kein Eindeutigkeits-Check in `addActivity()`) waren für Screenreader-Nutzer:innen nicht mehr auseinanderzuhalten. Fix analog `Reiseentwuerfe.tsx` (17.09.): bei Namensduplikaten "(Eintrag N)" ergänzt.
...
- [ ] 7.0 Trip lifecycle pages — Drafts, booked trips, cart, dashboard, calendar, budget, map, favorites, alerts
  ...
  - [x] 7.3 Implement draft actions: pause, duplicate, finalize, delete — added directly to `Reiseentwuerfe.tsx`'s local demo-draft state (pause/resume toggle, duplicate inserts a copy, finalize sets an "Abgeschlossen" status, delete removes the card); no separate action component, same inline pattern as the card itself. "Finalize" only changes the local status badge — there's no shared trip storage yet to move a draft into `MeineReisen.tsx`, so it doesn't attempt that (would be real persistence work tied to the still-open Backend-Entscheidung). Vom autonomen IT-Chef-Lauf am 17.09. (weiterer Lauf) nachgebessert: der Löschen-Button entfernte einen Entwurf bisher sofort ohne Rückfrage, anders als dasselbe Löschmuster auf `Preisalarme.tsx`/`Favoriten.tsx`/`Angebote.tsx`/`Aktivitaeten.tsx`/`Warenkorb.tsx` — jetzt fragt derselbe Bestätigungsdialog ("Entwurf löschen?"/"Ja, entfernen"/"Abbrechen") vor dem endgültigen Entfernen nach. Vom autonomen IT-Chef-Lauf am 17.09. (vierter Lauf) einen von `reports/support-chef.md` (17.09., Vorschlag 1) gemeldeten Fund behoben: Nach "Duplizieren" trugen Original und Kopie exakt dasselbe `aria-label` (nur aus `draft.destination` gebildet), waren also für Screenreader-Nutzer:innen nicht mehr unterscheidbar. Alle vier Aktions-Buttons (Pausieren/Fortsetzen, Abschließen, Duplizieren, Löschen) hängen jetzt bei Duplikaten zusätzlich "(Eintrag N)" an, wobei N die Position unter den gleichnamigen Entwürfen ist; ohne Duplikate bleibt das Label unverändert. Vom autonomen IT-Chef-Lauf am 20.09. einen von `reports/support-chef.md` (18.09., Vorschlag 3) gemeldeten Fund behoben: "Abschließen" setzte den Status bisher sofort und endgültig, ohne Rückfrage — anders als "Löschen" auf derselben Karte, das bereits über den etablierten Bestätigungsdialog abgesichert ist. Jetzt exakt dasselbe Muster: ein zweiter, per `pendingFinalize`-State gesteuerter Dialog ("Entwurf abschließen?"/"Ja, abschließen"/"Abbrechen"), kein neuer Entwurf.
  - [ ] 7.4 Implement "Planung fortsetzen" — resume KI-Chat with full history at interruption point — real implementation still blocked (needs a data model for multiple simultaneously stored chat histories, `tripStorage.ts` currently tracks only one active trip; own architecture decision, not autonomously decidable). Short-term mitigation added on 2026-09-11: `Reiseentwuerfe.tsx` now shows an honest dashed-border hint (visible only with 2+ non-finalized drafts, same pattern as the rewards-program hint in `Dashboard.tsx`) that "Planung fortsetzen" opens the one shared chat rather than that specific draft's details, per `reports/support-chef.md` (2026-09-10, Vorschlag 3); condition corrected on 2026-09-13 to count only non-finalized drafts, per `reports/support-chef.md` (2026-09-11, Fund 1); on 2026-09-22 the teal-highlighted "Planung fortsetzen" button itself is now hidden once a draft is `finalized` (it used to render for every draft regardless of status, inviting further planning on an explicitly finished trip right after the user confirmed "Ja, abschließen"), per `reports/support-chef.md` (2026-09-21, Vorschlag 2)
  ...
  - [x] 7.7 Build Dashboard page (`/dashboard`) with trips overview, budgets, favorites, loyalty points — `src/pages/Dashboard.tsx`, four stat tiles (bevorstehende Reisen, Reiseentwürfe-Fortschritt, Warenkorb-Summe, Favoriten) each linking to its full page, reusing the exact demo data/utilities from `MeineReisen.tsx`/`Reiseentwuerfe.tsx`/`Warenkorb.tsx`/`Favoriten.tsx` instead of a separate invented dataset. Loyalty points intentionally not shown as a number — FR-107/PRD open question OQ-04 ("exact rules for loyalty points accrual and redemption") is still unresolved, so an honest placeholder note explains that instead, per the "ehrlich statt erfunden" brand rule in `MARKENDESIGN.md`. No budget-vs-limit warning coloring, since `TripDraft` still has no real price fields to compare against (same gap as 6.6/6.7/7.12).
  ...
  - [x] 7.11 Build Reisekalender page (`/kalender`) with calendar view of all trips — vom autonomen IT-Chef-Lauf am 17.09. (weiterer Lauf) nachgebessert: die "Heute"-Markierung im Kalendergitter war bisher nur eine goldene Randfarbe, ohne Text-Alternative für Screenreader (anders als die Trip-Badges direkt darunter, für die dieselbe Seite bereits eine eigene textliche Liste hat, siehe Kommentar dort). Jetzt zusätzlich `aria-current="date"` auf der Tageszelle sowie ein `sr-only`-Zusatz "(Heute)" neben der Tageszahl, exakt nach dem in `ChecklistPanel.tsx` etablierten Muster.
